...
//                    animateCamera(CameraUpdateFactory.newLatLngZoom(hospitalInfo.getLatLng(), 15f), 1, this)
    ...
    fun onClickConsult(view: View) {
        ...
        afterCheckLoginDo { startActivity(Intent(applicationContext, RequestConsultActivity::class.java).putExtra("isOpen", false).putExtra("hospitalId", hospitalInfo.objectID).putExtra("name", hospitalInfo.name)) }
    }
}
